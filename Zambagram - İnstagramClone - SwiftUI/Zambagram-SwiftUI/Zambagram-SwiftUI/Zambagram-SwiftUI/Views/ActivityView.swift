        ...
        HStack {
            ...
            if activity.activity == .liked {
                Text(activity.getUsernames())
                    ...
                    Text(activity.usersInContext.count == 1 ? " gönderini beğendi. " : " ve diğerleri gönderini beğendi. ")
                ...
                Text(activity.getUsernames())
                    .font(Font.system(size: 12, weight: .semibold)) +
                    Text(" bir yorumda sizden bahsetti: ")
                    .font(Font.system(size: 12, weight: .medium)) +
                ...
                Text(activity.getUsernames())
                    .font(Font.system(size: 12, weight: .semibold)) +
                    Text(" Zambagram'da tanıyor olabilirsin. ")
                    .font(Font.system(size: 12, weight: .medium)) +
                    Text(activity.duration)
                    .font(Font.system(size: 10, weight: .light))
            }
...
struct LikedActivityView_Previews: PreviewProvider {
    static var previews: some View {
        ActivityView(activity: Activity(activity: .liked,
                                             ...
                                             post: Post(user: User(userName: "cengizui", userImage: "sample_post") , postImage: "post_18", caption: "If any one attempts to haul down the American flag, shoot him on the spot.", likes: "leomessi ve diğerleri beğendi")))
    }
}
